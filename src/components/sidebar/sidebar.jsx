...
{
  /*Icon Returning Function */
}
const getDashboardIcon = (sideSubMenu) => (
  <svg
    className={`${
      sideSubMenu ? "menu-item-icon-active" : "menu-item-icon-inactive"
    }`}
    width="24"
    height="24"
    viewBox="0 0 24 24"
...
const getCalenderIcon = (sideSubMenu) => (
  <svg
    className={`${
      sideSubMenu ? "menu-item-icon-active" : "menu-item-icon-inactive"
    }`}
...
const getProfileIcon = (sideSubMenu) => (
  <svg
    className={`${
      sideSubMenu ? "menu-item-icon-active" : "menu-item-icon-inactive"
    }`}
...
const getTablesIcon = (sideSubMenu) => (
  <svg
    className={`${
      sideSubMenu ? "menu-item-icon-active" : "menu-item-icon-inactive"
    }`}
...
const getPagesIcon = (sideSubMenu) => (
  <svg
    className={`${
      sideSubMenu ? "menu-item-icon-active" : "menu-item-icon-inactive"
    }`}
...
const getChartsIcon = (sideSubMenu) => (
  <svg
    className={`${
      sideSubMenu ? "menu-item-icon-active" : "menu-item-icon-inactive"
    }`}
...
const getUIElementsIcon = (sideSubMenu) => (
  <svg
    className={`${
      sideSubMenu ? "menu-item-icon-active" : "menu-item-icon-inactive"
    }`}
...
const getAuthenticationIcon = (sideSubMenu) => (
  <svg
    className={`${
      sideSubMenu ? "menu-item-icon-active" : "menu-item-icon-inactive"
    }`}
...
const sidebarMenu = [
  ...
  {
    label: "Calendar",
    getIcon: getCalenderIcon,
    path: "/calender",
  },
  {
    label: "User Profile",
    getIcon: getProfileIcon,
    path: "/profile",
  },
  {
    label: "Forms",
...
const sidebarMenuOthers = [
  {
    label: "Charts",
    getIcon: getChartsIcon,
    path: "/charts",
    subMenu: [
      {
        label: "Line Chart",
  ...
  {
    label: "User Profile",
    getIcon: getAuthenticationIcon,
    path: "/profile",
  },
];
const Sidebar = () => {
  const [activeIndexMain, setActiveIndexMain] = useState(null);
  const [activeIndexOther, setActiveIndexOther] = useState(null);

  const toggleSubMenuMain = (index) => {
    setActiveIndexMain(activeIndexMain === index ? null : index);
  };
  
  const toggleSubMenuOther = (index) => {
    setActiveIndexOther(activeIndexOther === index ? null : index);
  };
  
  return (
    <>
      <aside className="sidebar fixed left-0 top-0 z-9999 flex h-screen w-[290px] flex-col overflow-y-hidden border-r border-gray-200 bg-white px-5 dark:border-gray-800 dark:bg-black lg:static lg:translate-x-0">
        {/* SIDEBAR HEADER */}
          ...
          <nav>
            ...
            <div>
              <h3 className="mb-4 text-xs uppercase leading-[20px] text-gray-400">
                ...
                <svg
                  className="mx-auto fill-current menu-group-icon hidden"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
              ...
              <ul className="flex flex-col gap-4 mb-6">
                {sidebarMenu.map((data, index) => {
                 const isOpen = activeIndexMain === index;
                    ...
                    <li key={index} >
                      <a
                        ...
                        onClick={(e) => {
                          ...
                          toggleSubMenuMain(index);
                        }}
                      >
                        {data.getIcon(isOpen)}{" "}
                        {/* pass isOpen instead of sideSubMenu */}
                        <span className="menu-item-text">{data.label}</span>
            ...
            <div>
              <h3 className="mb-4 text-xs uppercase leading-[20px] text-gray-400">
                <span className="menu-group-title">others</span>
              ...
              <ul className="flex flex-col gap-4 mb-6">
                {sidebarMenuOthers.map((data, index) => {
                  const isOpen = activeIndexOther === index;

                  return (
                    <li key={index} onClick={() => toggleSubMenuOther(index)}>
                      <a
                        href="#"
                        className={`menu-item group ${
                          isOpen ? "menu-item-active" : "menu-item-inactive"
